...
def insert_data_into_director(cursor, director_id, director_name):
    if tabla_vacia(cursor,'director'):
...
def insert_data_into_productor(cursor, productor_id, productor_name):
    if tabla_vacia(cursor,'productor'):
...
def insert_data_into_productora(cursor, productora_id, productora_name):
    if tabla_vacia(cursor,'productora'):
...
def insert_data_into_finanzas(cursor, tiempo_id, director_id, productor_id, productora_id, coste, ingresos):
    if tabla_vacia(cursor,'finanzas'):
...
def insert_data_into_satisfaccion_usuarios(cursor, tiempo_votacion, tiempo_emision, director_id, productor_id, productora_id, votos, satisfaccion):
    cursor.execute("INSERT INTO satisfaccion_usuarios (tiempo_votacion, director, productor, productora, votos, satisfaccion) VALUES (%s, %s, %s, %s, %s, %s, %s)",
...
def print_xml(root,cursor=None, meses_del_ano = [
    "Enero",
    "Febrero",
    "Marzo",
    "Abril",
...
def print_csv(reader,cursor=None):
    for row in reader:
        # Trabaja con los datos de cada fila
         
        # Esto imprimirá cada fila del archivo CSV
        pelicula = row[0]
        usuario = row[1]
        instante = row[2]
        voto = row[3]
        if cursor:
            ...
            insert_data_into_satisfaccion_usuarios(cursor, tiempo_votacion, tiempo_votacion, directores_id, productor_id, productoras_id, voto, 0)

        else:
            print("csv",pelicula,usuario,instante,voto)
...
def load_file_tables(root,reader,cursor=None, meses_del_ano = [
    ...
    for pelicula in root.iter('pelicula'):
        ...
        productor_name,productor_id=get_item(pelicula,".//productor")
        fecha=emision.split('-')
        ano=fecha[0]
        tiempo_votacion=ano
        mes=fecha[1]
        mes_texto=meses_del_ano[int(mes)-1]
        if cursor:
            # POR CADA ID SE REGENERA PARA NO TENER QUE ESTAR
            # ALMACENANDO LAS ID AUTOGENERADOS 
            insert_data_into_tiempo(cursor, emision)
            insert_data_into_director(cursor, directores_id, directores_name)
            insert_data_into_productor(cursor, productor_id, productor_name)
        ...
        for row in reader:
            ...
            voto = row[3]
            echa=instante.split('-')
            tiempo_votacion=fecha[0]
            mes_v=fecha[1]
            if cursor:
                # Insertar los datos en la tabla de PostgreSQL
                insert_data_into_satisfaccion_usuarios(cursor, tiempo_votacion, directores_id, productor_id, productoras_id, voto, 0)
            else:
                print("csv",pelicula,usuario,instante,voto)
...
def generar_tabla(cursor):
    ...
    tablas = ["tiempo", "director", "productor", "productora","finanzas"]
...
def load_file(host,database,user,password,root_dir,xml_directory,csv_directory):
    ...
    for xml_filename in os.listdir(root_dir+xml_directory):
        ...
        #    load_file_tables(root,reader,meses_del_ano=meses_del_ano)
