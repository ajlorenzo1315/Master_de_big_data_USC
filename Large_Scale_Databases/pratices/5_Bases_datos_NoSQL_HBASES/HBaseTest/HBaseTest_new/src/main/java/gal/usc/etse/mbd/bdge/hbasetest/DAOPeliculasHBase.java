...
public class DAOPeliculasHBase implements DAOPeliculas {
    ...
    public List<Pelicula> getPeliculasPersonajeTitulo(int year, int month, int orden) {
        ...
        // Configuracion de las fechas de inicio y fin del mes especificado
            ...
            try (ResultScanner rs = pelsTable.getScanner(scan)) {
                for (Result filaPelicula = rs.next(); filaPelicula != null; filaPelicula = rs.next()) {
                    ...
                    for(Reparto elemento_reparto : reparto){
                        if(elemento_reparto.getOrden() <= orden){

                            reparto_orden.add( new Reparto(elemento_reparto.getOrden(),
                                elemento_reparto.getPersonaje(),
                                elemento_reparto.getIdPersona(),
                                elemento_reparto.getNombrePersona()));

                        }
                        System.out.print("Error al consultar la tabla de películas en HBase: " + elemento_reparto.getOrden() + "\n");
                    }
                    ...
                    resultado.add(new Pelicula(
                            ...
                            Bytes.toLong((byte[]) mapInfo.get(Bytes.toBytes("presupuesto"))),
                            Bytes.toLong((byte[]) mapInfo.get(Bytes.toBytes("ingresos"))),
                            reparto,//reparto_orden.toArray(new Reparto[0]),
                            null)
                    );
                    //reparto_orden.clear();
    ...
    public List<Pelicula> getPeliculasDirigidas(String nombrer) {
        List<Pelicula> resultado = new ArrayList<>();
        ...
        try {
            ...
            try (ResultScanner rs = pelsTable.getScanner(scan)) {
                for (Result filaPelicula : rs.next(); filaPelicula != null; filaPelicula = rs.next()) {
                    NavigableMap<byte[], byte[]> mapPersonal = filaPelicula.getFamilyMap(Bytes.toBytes("personal"));
                    boolean dirigidaPor = false;
                    for (Map.Entry<byte[], byte[]> elemPersonal : mapPersonal.entrySet()) {
                        Personal personal = (Personal) Utils.deserialize(elemPersonal.getValue());
                        if ("Director".equals(personal.getTrabajo()) && nombrer.equals(personal.getNombrePersona())) {
                            dirigidaPor = true;
                            break;
                        }
                    }
                    ...
                    if (dirigidaPor) {
                        // Procesar el reparto y el personal como en getPeliculasID
                        List<Reparto> repartoList = procesarReparto(filaPelicula);
                        List<Personal> personalList = procesarPersonal(filaPelicula);

                        NavigableMap<byte[], byte[]> mapInfo = filaPelicula.getFamilyMap(Bytes.toBytes("info"));
                        Pelicula pelicula = new Pelicula(
                            Bytes.toInt(mapInfo.get(Bytes.toBytes("id"))),
                            Bytes.toString(mapInfo.get(Bytes.toBytes("titulo"))),
                            new Date(Bytes.toLong(mapInfo.get(Bytes.toBytes("fechaEmision")))),
                            Bytes.toLong(mapInfo.get(Bytes.toBytes("presupuesto"))),
                            Bytes.toLong(mapInfo.get(Bytes.toBytes("ingresos"))),
                            repartoList.toArray(new Reparto[0]),
                            personalList.toArray(new Personal[0])
                        );
