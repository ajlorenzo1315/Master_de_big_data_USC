# Informaci�n de 10000 clientes de una entidad bancaria
# install.packages("ISLR")
library(ISLR)
data(Default)
head(Default)

# Objetivo: predecir Y (cuando un cliente incurrir� en impago 
# de cr�dito de la tarjeta)
...
# Si aplicamos regresi�n lineal cl�sica tenemos un problema
reglin <- lm(default01 ~ Default$balance)
plot(Default$balance,default01,
     main="Ajuste log�stico",xlab="Saldo",ylab="Impago",ylim=c(-0.1,1))
abline(reglin)


# Ajuste de un modelo de regresi�n log�stica
...
# La frontera entre clasificar como default o no, estar� en el valor
# de x que satisface
# beta_0 + beta_1 X = 0
...
# Comparamos realidad con clasificaci�n (Matriz de confusi�n)
table(Default$default,clasif1)

# Podr�a ser m�s "precavido" y catalogar como que sea
